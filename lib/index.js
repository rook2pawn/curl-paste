...
exports.writeFile = function(req,res,next) {
  ...
  .then(() => {
    ...
    console.log("from:", req.connection.address(), " gonig to write! " + protocol+'://'+hostname+'/id/'+id + '\nweb: '+protocol+'://'+hostname+'/web/'+id )
    return res.end(`raw: ${protocol}://${hostname}/id/${id}\nweb: ${protocol}://${hostname}/web/${id}\n`);
  })
  .catch((e) => {
...
exports.writeFileWeb = function(req,res,next) {
  ...
  .then((body) => {
    ...
    if (isSecure === undefined) {
      return store.write(id,text)
      .then(() => {
        res.writeHead(301, {'Location':'/web/'+id})
        res.end('\n');
      })
    } else {
      return store.writeViewOnce(id,text)
      .then(() => {
...
exports.writeFileViewOnce = function(req,res,next) {
  ...
  .then((body) => {
    ...
    return store.writeViewOnce(id,body)
    .then(() => {
      return res.end(`raw: ${protocol}://${hostname}/id/${id}\nweb: ${protocol}://${hostname}/web/${id}\n`);
    })
  })
